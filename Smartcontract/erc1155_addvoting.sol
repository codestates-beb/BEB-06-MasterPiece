...
contract masterpiece {
    ...
    function mintpieceNFT(string memory uri, uint256 sbtamount)
        public
        onlyOwner
        returns (bool)
    {
    ...
    function souldrop(
        ...
        require(
            _nftstatus[id] == 0,
            "Not minted SBT or Not correct NFT status"
        ); // status 및 mint 되었는지 확인
    ...
    //@ balance 조회
    function soulBalanceOf(uint256 id, address account)
        public
        view
        returns (uint256)
    {
    ...
    //@ sould uri 조회
    function getsouldUri(uint256 id, address account)
        public
        view
    ...
    //@ SBT 참여자 탈퇴 ( 소각 )
    function dropSoulowner(uint256 id, address addr)
        public
        onlyOwner
        returns (bool)
    {
    ...
    function piecetransfer(uint256 id, address newowner)
        public
        onlyOwner
        returns (bool)
    {
    ...
    function setStaking(uint256 pieceid, uint256 duration)
        public
        onlyOwner
        returns (uint256)
    {
        ...
        _staking[pieceid].rewardexpiredate = block.timestamp + duration + 60; // 60초 안에 수령 해야함
    ...
    function getStakingmoney(uint256 pieceid, address smAddr)
        public
        returns (uint256)
    {
        require(
            block.timestamp > _staking[pieceid].expiredate,
            "Woops! Please wait for the expiration date!"
        ); // 스테이킹 기간지난 이후 받음
        require(soulBalanceOf(pieceid, msg.sender) == 1, "Not owned Soul"); // soul 가진 사람만 수령 가능
        require(
            _staking[pieceid].stakingstatus == 1,
            "Woops! It is not on staking"
        ); // 스테이킹 status 가 1 일때만 받음, 제안상태에서 0으로 바꿔줌, 이후 스테키이세팅에서 1로 변경
        require(
            _gettingrewardstatus[pieceid][msg.sender] == 0,
            "Woops! Already got tge reward"
        ); // reward 보상 이력이 0 일때만 수령
    ...
    function _changeStatusstaking(uint256 pieceid) external {
        require(
            block.timestamp > _staking[pieceid].rewardexpiredate,
            "Woops! Can not change the staking status"
        ); // 보상 완료 받기전까지  변경 불가
...
contract DaoVoting {
    ...
    function getRight(
        ...
        uint256 pieceid,
        address account,
        uint256 proposernumber
    ) public returns (uint256) {
        require(
            masterpiece(mSMaddr).soulBalanceOf(pieceid, account) == 1,
            "Not owned Soul"
        ); //soul을 소유했는지 확인
        require(
            block.timestamp > proposers[pieceid][proposernumber].expiration,
            "Expired time getting voting right"
        );
    ...
    function isitRight(address account, uint256 pieceid)
        public
        view
        returns (uint256)
    {
    ...
    function suggestion(
        ...
        require(
            block.timestamp >
                masterpiece(mSMaddr)._getexpireddate(pieceid) + 60,
            "Woops! Under staking"
        ); // staking 완료 + 보상 수령 전에 제안 불가
        proposers[pieceid][_proposercounter[pieceid]].soulowner = account;
        proposers[pieceid][_proposercounter[pieceid]]
            .proposeuri = suggestionuri;
        ...
        proposers[pieceid][_proposercounter[pieceid]].expiration =
            block.timestamp +
            duration;

        _proposercounter[pieceid]++; //제안 번호 증가
        masterpiece(mSMaddr)._changeStatusstaking(pieceid);
        return _proposercounter[pieceid] - 1; // 제안번호
    }
    ...
    function voting(
        ...
        require(
            voters[account][pieceid].voted == false,
            "Woops! Already voted"
        );
        require(
            proposers[pieceid][proposedid].expiration >= block.timestamp,
            "Woops! Finish voting"
        );
        require(
            proposers[pieceid][proposedid].soulowner != address(0),
            "Woop! Nobody suggested the proposal"
        );
    ...
    function result(
        ...
        require(
            block.timestamp > proposers[pieceid][proposedid].expiration,
            "Woops! Not finished voting"
        );
        if (
            proposers[pieceid][proposedid].agree >
            proposers[pieceid][proposedid].disagree
            ...
            if (
                keccak256(
                    abi.encodePacked(proposers[pieceid][proposedid].proposeuri)
