...
module.exports = {
	...
	agenda: async (req, res) => {
		...
		const {collectionName, nftName, address, title, description, type, duration} = req.body;
		let query = `
            select n.id
            from nft n
		...
		db.query(query, [collectionName, nftName], (err, rows) => {
			...
			query = `
                ...
                values (?, ?, ?, ?, ?, ?, ?)
			`;
			db.query(query, [postId, nftId, address, title, description, type, duration], (err, rows) => {
				if (err) throw err;
				console.log(rows);
				res.send("ok");
			})
